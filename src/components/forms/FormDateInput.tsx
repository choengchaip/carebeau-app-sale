...
export const FormDateInput = (props: IFormInputProps) => {
  ...
  useEffect(() => {
    if (isShow) {
      if (!get(props.form.form, props.name, false)) {
        props.form.setForm({...props.form.form, [props.name]: moment().format('YYYY-MM-DD')})
  ...
  const getValue = () => {
    ...
    return moment(`${get(props.form.form, props.name)} 00:00:00`).toDate()
  }
        ...
        <Box
          ...
          <RNDateTimePicker
            ...
            onChange={(_: any, date: any) => {
              setIsConfirm(false)
              props.form.setForm({...props.form.form, [props.name]: moment(date).format('YYYY-MM-DD')})
              props.form.setError({...props.form.error, [props.name]: undefined })
          ...
          <Pressable onPress={() => {
            ...
            <HStack
              ...
              <Text
                ...
                mr={6}>
                {TimeUtil.fromDateToDateTH(get(props.form.form, props.name) || props.placeholder || 'วว-ดด-พศ')}
              </Text>
